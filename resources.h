...
#define RESOURCES
//----------------------------------------------------------------
#define TITLE                                       "NS v0.5.48 08/01/2016"
#define ICON_APP                                    100
//----------------------------------------------------------------
...
#define SSH_ERROR                 -1
//#define SSH_ERROR_CONNECT         -2
#define SSH_ERROR_PUBKEY          -3
#define SSH_ERROR_PUBKEY_HACK     -4
#define SSH_ERROR_PUBKEY_UNKNOW   -5
#define SSH_ERROR_PUBKEY_ADD      -6

#define SSH_ERROR_OPEN_CHANNEL    -10
#define SSH_ERROR_EXEC_CHANNEL    -11
#define SSH_ERROR_READ_CHANNEL    -12
#define SSH_ERROR_BAD_MEM_CHANNEL -13

#define SSH_ERROR_AUTH            -20

#define SSH_TIMEOUT_MAX           5 //5 secondes

#define SSH_OS_SZ_MAX              80
//----------------------------------------------------------------
...
#define HASH_TYPE_SHA256          3

#define HASH_MAX_SIZE             2048
//-------------------------------------
#define FILES_TYPE_EXT            1   //ext
#define FILES_TYPE_ST_CH          2   //start of string
#define FILES_TYPE_ED_CH          4   //end of string
#define FILES_TYPE_IN_CH          8   //in the string
#define FILES_TYPE_CH             16  //equal string
#define FILES_TYPE_CH_NO_EXT      32  //equal string without ext

#define FILES_ST_MAX_SIZE         256
//-------------------------------------
#define FSIZE_TYPE_EQ             1
#define FSIZE_TYPE_MIN            2
#define FSIZE_TYPE_MAX            3
#define FSIZE_TYPE_MINMAX         4

//-------------------------------------
#define DATAS_TYPE_ST             1   //string direct in file
#define DATAS_TYPE_ST_START       2   //string in the start of file
#define DATAS_TYPE_ST_END         3   //string in the end of file
#define DATAS_TYPE_STUK           4   //string format with ?/* for unknow char or string
#define DATAS_TYPE_BIN            5   //hexa format with ?? for unknow format

#define DATAS_MAX_SIZE            512
//-------------------------------------
#define SFSIGN_MAX_SZ             128
#define IOC_INFO_MAX_SZ           128

#define MAX_ITEM_BY_IOC           10
//----------------------------------------------------------------
typedef struct
{
  int mandatory;
  unsigned int type;                  //MD5, SHA1, SHA256
...
typedef struct
{
  int mandatory;
  unsigned int type;
  char file[FILES_ST_MAX_SIZE];
  //regex to check : [0-9]{0,10} or[a-z]{0,10}
}SFILES;

typedef struct
{
  int mandatory;
  char sign[SFSIGN_MAX_SZ];
}SFSIGN;

typedef struct
{
  //int mandatory;   //mandatory by default if enable
  unsigned int type;
  LONGLONG file_size;
  LONGLONG file_size2;
}SFSIZE;
...
typedef struct
{
  int mandatory;
  unsigned int type;
  char datas[DATAS_MAX_SIZE];
}SDATAS;

typedef struct
{
  char infos1[IOC_INFO_MAX_SZ];
  char infos2[IOC_INFO_MAX_SZ];
  char infos3[IOC_INFO_MAX_SZ];

  unsigned int nb_hash;
  SHASH hash[MAX_ITEM_BY_IOC];

  unsigned int nb_files;
  SFILES files[MAX_ITEM_BY_IOC];

  unsigned int nb_sign;
  SFSIGN sign[MAX_ITEM_BY_IOC];

  unsigned int nb_size;
  SFSIZE size[MAX_ITEM_BY_IOC];

  unsigned int nb_datas;
  SDATAS datas[MAX_ITEM_BY_IOC];
}S_IOC;

typedef struct
{
  unsigned int nb_ioc;
  unsigned int nb_ioc_mem;
  S_IOC *lioc;
}SIOC;
SIOC all_ioc;
BOOL OK_IOC;
...
void FileToSHA1(HANDLE Hfic, char *csha256);
void FileToSHA256(HANDLE Hfic, char *csha256);
void FileToSHA256only(HANDLE Hfic, char *csha256);
...
//SSH
int LinuxStart_msgOK(char *msg, char*cmd);
